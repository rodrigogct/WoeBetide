...
from Inventory.models import Collection, Garment, Sale, SaleItem, Payment
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for index, row in df.iterrows():
            ...
            # Payment is sale-level, so create/update it only once per Sale ID.
        ...
        self.stdout.write(f"Created garments: {created_garments}")
        self.stdout.write(f"Updated garments: {updated_garments}")
        self.stdout.write(f"Created sales: {created_sales}")
        self.stdout.write(f"Created/updated payments: {created_payments}")